...
export function shortItemName(name){
  const m=name.match(/^Пробковый сбор — (.+)$/);
  if(!m)return name;
  const lbl=m[1].replace('Пиво / Лимонады / Напитки','Пиво/Лимонады').replace('Вино / Шампанское','Вино').replace('Крепкий алкоголь','Крепкий');
  return'🍾 Пробка · '+lbl;
}
...
export function renderDeliveryLogSidebar(){
  ...
  if(!entries.length){el.innerHTML='<div class="sidebar-log-empty">Пусто за сутки</div>';return;}
  el.innerHTML=entries.map(e=>`<div class="sidebar-log-row"><span class="sidebar-log-time">${fmt(e.at)}</span><span class="sidebar-log-text" title="${escAttr(e.name)} · #${escAttr(e.orderNum||'?')} · Стол ${escAttr(e.table)} · ${escAttr(e.qty)}шт"><b>${esc(e.table)}</b> · ${esc(shortItemName(e.name||''))} <span>×${esc(e.qty)}</span></span></div>`).join('');
}
...
export function renderTables(){
  ...
  if(qnEl)qnEl.innerHTML=allDates.map(d=>`<button onclick="jumpDate('${escAttr(d)}')" class="date-chip${d===S.viewDate?' active':''}">${dateLbl(d)}</button>`).join('');
  ...
  document.getElementById('tablesBillList').innerHTML=sessions.map(({tNum,sid,orders:tOrdersRaw,meta})=>{
    ...
    const ordersHtml=(()=>{
      ...
      const renderOrder=o=>{
        ...
        const lines=(o.items||[]).map(it=>`<div class="tbo-line${it.status==='done'?' tl-done':''}"><span class="tl-name">${esc(shortItemName(it.name))}</span><span class="tl-qty">${it.qty} шт.</span></div>`).join('');
        return`<div class="tbo-item"><div class="tbo-hdr"><span class="tbo-num">#${esc(o.num)} ${sico}</span><span class="tbo-time">${fmt(o.createdAt)}</span><button class="btn-edit tbo-edit-btn" data-action="edit" data-oid="${escAttr(o.id)}" data-bill="1">✏️</button></div><div class="tbo-lines">${lines}</div>${note}</div>`;
      };
      let html=before.map(renderOrder).join('');
      if(after.length)html+=`<div class="tbo-after-log">🆕 Дозаказ после внесения в систему (${fmt(loggedAt)})</div>`+after.map(renderOrder).join('');
      return html;
    })();
    const closedAt=isCurrentSession?meta.closedAt:undefined;
    const closedLbl=!isOpen&&closedAt?`<span class="tb-closed-time">Оплачен в ${fmt(closedAt)}</span>`:'';
    const totalItems=tOrders.reduce((s,o)=>s+(o.items?o.items.reduce((a,i)=>a+i.qty,0):0),0);
    const safeDate=escAttr(S.viewDate),safeT=escAttr(tNum),safeSid=escAttr(sid);
